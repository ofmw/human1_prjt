...
//사용자가 입력한 검색 조건을 Map에 저장
    ...
    List<CommentVo> cList = dao.selectCommentAll(map);//게시물 목록
    ...
    */
    ...
    int totalRows = dao.countCommentAll(map);//총 레코드 수
    dao.close();
    int rows_per_page = 0;
        ...
        <c:choose>
            ...
            <c:otherwise>
                <c:forEach var="rowNum" begin="${startNum}" end="${endNum}">
                    <tr>
                        <td><div id="div_title"><a href="../learncoding2/qa3.jsp?no=${cList[rowNum-1].b_idx}">${cList[rowNum-1].b_title}</a></div></td>
                        <td><div id="div_content"><a href="../learncoding2/qa3.jsp?no=${cList[rowNum-1].b_idx}">${cList[rowNum-1].content}</a></div></td>
                        <td id="td_postDate"><fmt:formatDate value="${cList[rowNum-1].post_date}" pattern="yyyy-MM-dd" /></td>
                    </tr>
