...
<%
    ...
    try{
        ...
        int result = dao.updatePreset(vo);
        dao.close();
        
        if(result == 1){//글 수정 성공시
            JSFunction.alertLocation("글 수정이 완료되었습니다.", "../admin3_ps.jsp?no="+vo.getB_idx(), out);
        }else{//글 수정 실패시
            JSFunction.alertBack("글 수정에 실패했습니다.", out);
        }
