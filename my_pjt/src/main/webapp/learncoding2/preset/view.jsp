...
function deletePost() {
    ...
    if (confirmAns) {
        ...
        frm_view.action = "preset/delete_process.jsp?no=${boardFileVo.b_idx}"; // 전송 경로
    	...
    	<tr>
            <td colspan="4" id="td_btn" style="text-align: right; border: none;">
               <c:if test="${(!empty member) and (member.m_idx eq boardFileVo.m_idx)}" >
                    <input type="button" onclick="location.href='preset4.jsp?no=${boardFileVo.b_idx}'" value="수정하기">
                    <input type="button" onclick="deletePost();" value="삭제하기">
                </c:if>                
                <input type="button" onclick="location.href='preset.jsp';" value="목록보기">
            </td>
        </tr>
        <tr>
            <th>글 번호</th>
